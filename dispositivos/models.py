...
class Dispositivo(models.Model):
    ...
    zona = models.ForeignKey(Zona, on_delete=models.PROTECT, related_name="dispositivos", default=1)  # Aquí se asigna un valor por defecto
...
class Medicion(models.Model):
    ...
    consumo_w = models.DecimalField(max_digits=10, decimal_places=2)

    dispositivo = models.ForeignKey(
        Dispositivo, on_delete=models.PROTECT, related_name="mediciones"
    )

    def __str__(self):
        return f"Medición {self.id} - {self.fecha} - {self.dispositivo.nombre}"
